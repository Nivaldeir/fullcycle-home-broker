...
import { WalletAsset as WalletAssetSchema } from './wallet_asset.schema';
...
export class WalletAssetsService {
    ...
    subscribeEvents({ wallet_id }: { wallet_id: string; }): Observable<{ event: "wallet-asset-updated", data: WalletAsset }> {
        return new Observable((observer) => {
            this.walletAssetModel.watch([
            ...
            ], { fullDocument: "updateLookup" }).on("change", async (data) => {
                console.log(data)
                ...
                observer.next({ event: "wallet-asset-updated", data: walletAsset });
            });
        });
    }
}
